...
class RealtimeClient:
    ...
    async def stream_audio(self, audio_chunk: bytes) -> None:
        ...
        # Every ~500ms (assuming 24kHz, 16-bit, mono audio with chunk size 1024),
        # send a commit message to ensure audio is processed in reasonable chunks
        self._audio_chunks_sent = getattr(self, "_audio_chunks_sent", 0) + 1
        if self._audio_chunks_sent >= 10:  # Commit every ~10 chunks
            commit_event = {
                "type": "input_audio_buffer.commit"
            }
            await self.ws.send(json.dumps(commit_event))
